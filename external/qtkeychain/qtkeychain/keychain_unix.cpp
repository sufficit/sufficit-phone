...
static bool isKwalletAvailable(const char *dbusIface, const char *dbusPath) {
	...
	org::kde::KWallet iface(QLatin1String(dbusIface), QLatin1String(dbusPath), QDBusConnection::sessionBus());
...
static void kwalletReadPasswordScheduledStartImpl(const char *service, const char *path, ReadPasswordJobPrivate *priv) {
	if (QDBusConnection::sessionBus().isConnected()) {
		priv->iface =
		    new org::kde::KWallet(QLatin1String(service), QLatin1String(path), QDBusConnection::sessionBus(), priv);
		const QDBusPendingReply<QString> reply = priv->iface->networkWallet();
		auto watcher = new QDBusPendingCallWatcher(reply, priv);
		priv->connect(watcher, &QDBusPendingCallWatcher::finished, priv, &ReadPasswordJobPrivate::kwalletWalletFound);
	} else {
	...
	switch (getKeyringBackend()) {
		...
		case Backend_GnomeKeyring:
			...
			if (!GnomeKeyring::find_network_password(
			        ...
			        reinterpret_cast<GnomeKeyring::OperationGetStringCallback>(&JobPrivate::gnomeKeyring_readCb), this,
			        nullptr))
				q->emitFinishedWithError(OtherError, tr("Unknown error"));
			break;
...
void JobPrivate::kwalletWalletFound(QDBusPendingCallWatcher *watcher) {
	...
	connect(pendingWatcher, &QDBusPendingCallWatcher::finished, this, &JobPrivate::kwalletOpenFinished);
}
...
void JobPrivate::gnomeKeyring_readCb(int result, const char *string, JobPrivate *self) {
	if (result == GnomeKeyring::RESULT_OK) {
		if (self->mode == JobPrivate::Text) self->data = QByteArray(string);
		else self->data = QByteArray::fromBase64(string);

		self->q->emitFinished();
	} else if (self->mode == JobPrivate::Text) {
		self->mode = JobPrivate::Binary;
		if (!GnomeKeyring::find_network_password(
		        self->key.toUtf8().constData(), self->q->service().toUtf8().constData(), "base64",
		        reinterpret_cast<GnomeKeyring::OperationGetStringCallback>(&JobPrivate::gnomeKeyring_readCb), self,
		        nullptr))
	...
	if (q->insecureFallback() && plainTextStore.contains(key)) {
		...
		if (err.type() == QDBusError::ServiceUnknown) // KWalletd not running
			...
			q->emitFinishedWithError(
			    OtherError,
			    tr("Could not open wallet: %1; %2").arg(QDBusError::errorString(err.type()), err.message()));
	}
}
...
void ReadPasswordJobPrivate::kwalletOpenFinished(QDBusPendingCallWatcher *watcher) {
	...
	if (plainTextStore.contains(key)) {
		...
		if (mode == WritePasswordJobPrivate::Binary) j->setBinaryData(data);
		else if (mode == WritePasswordJobPrivate::Text) j->setTextData(QString::fromUtf8(data));
		else Q_ASSERT(false);
	...
	connect(nextWatcher, &QDBusPendingCallWatcher::finished, this, &ReadPasswordJobPrivate::kwalletEntryTypeFinished);
}
...
void ReadPasswordJobPrivate::kwalletEntryTypeFinished(QDBusPendingCallWatcher *watcher) {
	...
	if (watcher->isError()) {
		...
		q->emitFinishedWithError(
		    OtherError,
		    tr("Could not determine data type: %1; %2").arg(QDBusError::errorString(err.type()), err.message()));
		return;
	}
	...
	const auto nextReply =
	    (mode == Text) ? QDBusPendingCall(iface->readPassword(walletHandle, q->service(), key, q->service()))
	    : (mode == JobPrivate::Map) ? QDBusPendingCall(iface->readMap(walletHandle, q->service(), key, q->service()))
	                                : QDBusPendingCall(iface->readEntry(walletHandle, q->service(), key, q->service()));
	auto nextWatcher = new QDBusPendingCallWatcher(nextReply, this);
	connect(nextWatcher, &QDBusPendingCallWatcher::finished, this, &ReadPasswordJobPrivate::kwalletFinished);
}
...
static void kwalletWritePasswordScheduledStart(const char *service, const char *path, JobPrivate *priv) {
	if (QDBusConnection::sessionBus().isConnected()) {
		...
		const QDBusPendingReply<QString> reply = priv->iface->networkWallet();
		auto watcher = new QDBusPendingCallWatcher(reply, priv);
		priv->connect(watcher, &QDBusPendingCallWatcher::finished, priv, &JobPrivate::kwalletWalletFound);
	} else {
		// D-Bus is not reachable so none can tell us something about KWalletd
	...
	switch (getKeyringBackend()) {
		...
		case Backend_GnomeKeyring: {
			...
			if (!GnomeKeyring::store_network_password(
			        GnomeKeyring::GNOME_KEYRING_DEFAULT, descr.toUtf8().constData(), key.toUtf8().constData(),
			        service.constData(), type.toUtf8().constData(), password.constData(),
			        reinterpret_cast<GnomeKeyring::OperationDoneCallback>(&JobPrivate::gnomeKeyring_writeCb), this,
			        nullptr))
				q->emitFinishedWithError(OtherError, tr("Unknown error"));
		} break;
...
void WritePasswordJobPrivate::fallbackOnError(const QDBusError &err) {
	if (!q->insecureFallback()) {
		q->emitFinishedWithError(
		    OtherError, tr("Could not open wallet: %1; %2").arg(QDBusError::errorString(err.type()), err.message()));
		return;
	}
