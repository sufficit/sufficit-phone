...
KeyPairGenerator KeyPairGenerator::getInstance(const QString &algorithm, const QString &provider) {
	return handleExceptions(
	    callStaticObjectMethod("java/security/KeyPairGenerator", "getInstance",
...
bool KeyStore::containsAlias(const QString &alias) const {
	return handleExceptions(callMethod<jboolean>("containsAlias", "(Ljava/lang/String;)Z", fromString(alias).object()));
}
...
KeyGenParameterSpec::Builder KeyGenParameterSpec::Builder::setEncryptionPadding(const QString &padding) const {
	QAndroidJniEnvironment env;
	...
	const KeyGenParameterSpec::Builder result = handleExceptions(callObjectMethod(
	    "setEncryptionPaddings", "([Ljava/lang/String;)Landroid/security/keystore/KeyGenParameterSpec$Builder;", arr));
	env->DeleteLocalRef(arr);
	return result;
}

KeyGenParameterSpec::Builder KeyGenParameterSpec::Builder::setKeySize(int keySize) const {
	return handleExceptions(callObjectMethod("setKeySize", "(I)Landroid/security/keystore/KeyGenParameterSpec$Builder;",
...
ByteArrayOutputStream::ByteArrayOutputStream() : OutputStream(QAndroidJniObject("java/io/ByteArrayOutputStream")) {
	handleExceptions();
}
...
bool Cipher::init(int opMode, const Key &key, const java::security::spec::AlgorithmParameterSpec &params) const {
	callMethod<void>("init", "(ILjava/security/Key;Ljava/security/spec/AlgorithmParameterSpec;)V", opMode, key.object(),
	                 params.object());
	return handleExceptions();
}
...
bool Cipher::doFinal(const QByteArray &input, QByteArray &output, QString *errorString) const {
	...
	env->DeleteLocalRef(cls);
	Q_ASSERT(method);
	const jobject resultObj = env->CallObjectMethod(obj, method, toArray(input).object());
	if (env->ExceptionCheck()) {
		const jthrowable exception = env->ExceptionOccurred();
		env->ExceptionClear();
		if (errorString && exception) {
...
SecureRandom::SecureRandom() : Object(QAndroidJniObject("java/security/SecureRandom")) {
	handleExceptions();
}
...
SecretKeySpec::SecretKeySpec(const QByteArray &key, const QString &algorithm)
    : Key(QAndroidJniObject("javax/crypto/spec/SecretKeySpec",
                            ...
                            toArray(key).object(),
                            fromString(algorithm).object())) {
	handleExceptions();
}

GCMParameterSpec::GCMParameterSpec(int tLen, const QByteArray &iv)
    : AlgorithmParameterSpec(QAndroidJniObject(
          "javax/crypto/spec/GCMParameterSpec", "(I[B)V", static_cast<jint>(tLen), toArray(iv).object())) {
	handleExceptions();
}
