		...
		if (error) {
			...
			if (password) {
				...
				secret_password_lookup_fn(qtkeychainSchema(), nullptr, on_password_lookup, arg, "user",
				                          arg->user.toUtf8().constData(), "server", arg->server.toUtf8().constData(),
				                          "type", "base64", nullptr);
				return;
			} else {
...
bool LibSecretKeyring::isAvailable() {
	...
	if (!keyring.isLoaded()) return false;
	if (secret_password_lookup_fn == nullptr) return false;
	if (secret_password_lookup_finish_fn == nullptr) return false;
	if (secret_password_store_fn == nullptr) return false;
	if (secret_password_store_finish_fn == nullptr) return false;
	if (secret_password_clear_fn == nullptr) return false;
	if (secret_password_clear_finish_fn == nullptr) return false;
	if (secret_password_free_fn == nullptr) return false;
	if (secret_error_get_quark_fn == nullptr) return false;
	return true;
#else
	return false;
#endif
...
bool LibSecretKeyring::findPassword(const QString &user, const QString &server, QKeychain::JobPrivate *self) {
#if defined(HAVE_LIBSECRET)
	if (!isAvailable()) {
		return false;
	}
	...
	secret_password_lookup_fn(qtkeychainSchema(), nullptr, on_password_lookup, arg, "user", user.toUtf8().constData(),
	                          "server", server.toUtf8().constData(), "type", "plaintext", nullptr);
	return true;
#else
	Q_UNUSED(user)
	Q_UNUSED(server)
...
bool LibSecretKeyring::writePassword(const QString &display_name,
	...
	secret_password_store_fn(qtkeychainSchema(), SECRET_COLLECTION_DEFAULT, display_name.toUtf8().constData(),
...
bool LibSecretKeyring::deletePassword(const QString &key, const QString &service, QKeychain::JobPrivate *self) {
#if defined(HAVE_LIBSECRET)
	if (!isAvailable()) {
		return false;
	}

	secret_password_clear_fn(qtkeychainSchema(), nullptr, on_password_cleared, self, "user", key.toUtf8().constData(),
	                         "server", service.toUtf8().constData(), nullptr);
	return true;
#else
	Q_UNUSED(key)
	Q_UNUSED(service)
