...
void ReadPasswordJobPrivate::scheduledStart() {
	...
	if (encryptedData.startsWith(kHybridMagic)) {
		...
		if (!rsaCipher.doFinal(encryptedKey, aesKeyBytes, &decryptError)) {
			q->emitFinishedWithError(Error::OtherError, tr("Could not decrypt AES key: %1").arg(decryptError));
			return;
		}
		...
		if (!aesCipher || !aesCipher.init(Cipher::DECRYPT_MODE, aesKey, gcmSpec)) {
			q->emitFinishedWithError(Error::OtherError, tr("Could not create AES decryption cipher"));
			return;
		}

		if (!aesCipher.doFinal(encryptedPayload, plainData, &decryptError)) {
			q->emitFinishedWithError(Error::OtherError, tr("Could not decrypt data: %1").arg(decryptError));
			return;
		}
	} else {
...
void WritePasswordJobPrivate::scheduledStart() {
	...
	if (!keyStore.containsAlias(alias)) {
		const KeyGenParameterSpec spec =
		    KeyGenParameterSpec::Builder(alias, android::security::keystore::KeyProperties::PURPOSE_ENCRYPT |
		                                            android::security::keystore::KeyProperties::PURPOSE_DECRYPT)
		        .setEncryptionPadding(QStringLiteral("PKCS1Padding"))
		        .setKeySize(2048)
		        .build();

		const auto generator = KeyPairGenerator::getInstance(QStringLiteral("RSA"), QStringLiteral("AndroidKeyStore"));
	...
	if (!aesCipher.doFinal(data, encryptedPayload, &encryptError)) {
		q->emitFinishedWithError(Error::OtherError, tr("Could not encrypt data: %1").arg(encryptError));
		return;
	}
	...
	if (!rsaCipher.doFinal(aesKeyBytes, encryptedKey, &encryptError)) {
		q->emitFinishedWithError(Error::OtherError, tr("Could not encrypt AES key: %1").arg(encryptError));
		return;
	}
	...
	QByteArray blob;
	blob.reserve(kHybridMagic.size() + 4 + encryptedKey.size() + iv.size() + encryptedPayload.size());
	blob += kHybridMagic;
