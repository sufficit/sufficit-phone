...
inline T Object::handleExceptions(const T &result, const T &resultOnError) {
	if (!handleExceptions()) return resultOnError;
...
public:
	...
	bool
	init(int opMode, const java::security::Key &key, const java::security::spec::AlgorithmParameterSpec &params) const;
	bool doFinal(const QByteArray &input, QByteArray &output, QString *errorString = nullptr) const;
};
