...
static QString targetName(const QString &service, const QString &key) {
	if (useCompatNamingScheme()) return key;

	if (key.isEmpty()) return service;
	if (service.isEmpty()) return key;
	return key + '@' + service;
}
...
std::pair<QByteArray, QString> protectData(const QByteArray &data) {
	...
	if (!CryptProtectData(&blob_in, ENCRYPTED_DATA_KEY.data(), nullptr, nullptr, nullptr, 0, &blob_out)) {
...
void WritePasswordJobPrivate::scheduledStart() {
		...
		// credentials The data is also protected with the roaming profile
		{
			...
			if (result.first.size() > MAX_BLOB_SIZE) {
				q->emitFinishedWithError(
				    OtherError, tr("Credential size exceeds maximum size of %1: %2")
				                    .arg(QString::number(MAX_BLOB_SIZE), QString::number(result.first.size())));
				return;
			}
	...
	if (err == RPC_S_INVALID_BOUND) {
		...
		if (target.size() > maxTargetName) {
			q->emitFinishedWithError(OtherError, tr("Credential key exceeds maximum size of %1").arg(maxTargetName));
			return;
		}
	}
